...
func main() {
	...
	// 创建三个请求参数
	q1 := zcache.NewQuery("test", zcache.WithQueryMeta("world"))
	q2 := zcache.NewQuery("test", zcache.WithQueryMeta("world2"))
	q3 := zcache.NewQueryConfig().Bucket("test").Meta("world3").Make() // 等效于 NewQuery
